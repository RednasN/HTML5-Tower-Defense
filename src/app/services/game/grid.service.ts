...
export class GridService {
  ...
  public drawAssets(): void {
    if (this.selectedCell !== null) {
      const drawAssetx = this.selectedCell.drawx + this.canvasService.mainCanvasXOffset;
      const drawAssety = this.selectedCell.drawy + this.canvasService.mainCanvasYOffset;
    ...
    if (this.grid_height !== null && this.grid_width !== null) {
      for (let x = 0; x < this.grid_width; x++) {
        for (let y = 0; y < this.grid_height; y++) {
          ...
          if (imageIndex !== null) {
            this.render(
              this.imageService.gridImages[imageIndex],
              this.grid[x][y].drawx + this.canvasService.mainCanvasXOffset,
              this.grid[x][y].drawy + this.canvasService.mainCanvasYOffset
            );
          }
        }
      }
  ...
  public render(image: CanvasImageSource | null, x: number, y: number): void {
    if (image !== null) {
      this.canvasService.mainCtx.drawImage(image, x, y);
    }
  }
