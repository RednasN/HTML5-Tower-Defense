...
export class NuclearBullet extends Projectile {
  ...
  public calculate(): void {
    if (this.needdraw) {
      ...
      if (Math.abs(this.x - centerEnemyX) < 10 && Math.abs(this.y - centerEnemyY) < 10) {
        ...
        const routeIndex = this.twdGrid.enemies[this.enemyIndex].routeindex;

        const explosionx = this.twdGrid.route[routeIndex].drawx;
        const explosiony = this.twdGrid.route[routeIndex].drawy;

        this.twdGrid.explosions.push(new Explosion(this.canvasState, this.twdImages, this.twdGrid, explosionx, explosiony, 1));

        const routeLength = this.twdGrid.route;

        let startRouteIndexPlus = routeIndex - 1;
        let startRouteIndexMinus = routeIndex + 1;

        for (let i = 0; i < 6; i++) {
          if (startRouteIndexPlus < routeLength.length && startRouteIndexPlus > 0) {
            const explosionx = this.twdGrid.route[startRouteIndexPlus].drawx;
            const explosiony = this.twdGrid.route[startRouteIndexPlus].drawy;
            this.twdGrid.explosions.push(new Explosion(this.canvasState, this.twdImages, this.twdGrid, explosionx, explosiony, 1));
          }

          if (startRouteIndexMinus >= 0 && routeLength.length > 0) {
            const explosionx = this.twdGrid.route[startRouteIndexMinus].drawx;
            const explosiony = this.twdGrid.route[startRouteIndexMinus].drawy;
            this.twdGrid.explosions.push(new Explosion(this.canvasState, this.twdImages, this.twdGrid, explosionx, explosiony, 1));
          }
