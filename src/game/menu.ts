...
export class TwdMenu {
            ...
            for (let x = 0; x < this.twdGrid.grid.length; x++) {
                for (let y = 0; y < this.twdGrid.grid[x].length; y++) {
                    ...
                    if (this.canvasState.mainCanvasXCurrent > startx  && this.canvasState.mainCanvasXCurrent < (startx + this.twdGrid.grid[x][y].width) &&
                        ...
                        {
                             var turret = new Turret(this.twdImages, this.canvasState, this.twdGrid, this.twdGameLoop, x, y, 0);
    ...
    public checkMenuClick(): void
	{
		if(this.canvasState.mainCanvasXCurrent > this.canvasState.mainCanvas.width-this.twdImages.imgBlackTransparent!.width)
        {
        	for (let x = 0; x < this.sideMenu.length; x++) {
        		if(this.canvasState.mainCanvasXCurrent > this.sideMenu[x].drawx && this.canvasState.mainCanvasXCurrent < (this.sideMenu[x].drawx + this.twdImages.imgBlackTransparent!.width) 
        			&& this.canvasState.mainCanvasYCurrent > this.sideMenu[x].drawy && this.canvasState.mainCanvasYCurrent < (this.sideMenu[x].drawy + this.twdImages.imgBlackTransparent!.height))
        		{
    ...
    public draw(): void 
    {
        var fitVerticalCount = Math.floor(this.canvasState.mainCanvas.height / this.twdImages.imgBlackTransparent!.height);
        var startSideMenuY = Math.round((this.canvasState.mainCanvas.height - (fitVerticalCount * this.twdImages.imgBlackTransparent!.height)) / 2);   
        ...
        for (let i = 0; i < fitVerticalCount; i++) { 
            ...
            this.canvasState.mainCtx.drawImage(this.twdImages.imgBlackTransparent!, this.canvasState.mainCanvas.width-this.twdImages.imgBlackTransparent!.width, startSideMenuY);
            ...
            if(i != 0 && i != fitVerticalCount-1)
            {
                ...
                {
                    this.canvasState.mainCtx.drawImage(this.twdImages.towers[towerIndex + this.startIndex].images[0], this.canvasState.mainCanvas.width-this.twdImages.imgBlackTransparent!.width, startSideMenuY);
        ...
        if(this.draggedTurret != null)
        {	
            this.canvasState.mainCtx.drawImage(this.twdImages.towers[this.draggedTurret.turretIndex].images[0], this.canvasState.mainCanvasXCurrent -  this.twdImages.imgBlackTransparent!.width / 2, this.canvasState.mainCanvasYCurrent - this.twdImages.imgBlackTransparent!.height / 2);
        }			
};
}