import { TwdCanvasState } from "./canvas-state";

const TO_RADIANS = Math.PI / 180;

export type RotateImage = {
    name: string;
    images: HTMLImageElement[];
}


export type Image = {
    image: CanvasImageSource;
    imageb64: string | null;
}

export class TowerDefenseImages {

    public towers: RotateImage[]= [];
    public bullets: RotateImage[]= [];

    private enemies: RotateImage[]= [];
    private explosions: RotateImage[]= [];

    public gridImages : HTMLImageElement[] = [];
    public imgSurrounder: HTMLImageElement | null = null;
    public imgBlackTransparent: HTMLImageElement & CanvasImageSource | null = null;

    public async setupTowers(): Promise<void> {
        await Promise.all([
            await this.calculateRotateImagesAsync("BasicTower", imgTurret_Basic, this.towers),
            await this.calculateRotateImagesAsync("RocketLauncher", imgRocketLauncher_Basic, this.towers),
            await this.calculateRotateImagesAsync("NuclearLauncher", nuclear_turret, this.towers),
            await this.calculateRotateImagesAsync("MultiRocketLauncher", multilauncher, this.towers),
        ]);

        this.imgBlackTransparent!

        this.addTower(imgTurret_Basic, 0, 25);        
		this.addTower(imgRocketLauncher_Basic, 1, 100);
		this.addTower(nuclear_turret, 2, 100);
		this.addTower(multilauncher, 3, 100);

        console.log('Finished loading Towers');
    }

    public async setupEnemies(): Promise<void> {
		await this.calculateRotateImagesAsync("BasicEnemy", imgEnemy_Basic, this.enemies);

        console.log('Finished loading Enemies');
	};

    public async setupGridImages(): Promise<void> {
        for(var i = 0; i < gridImages.length; i++)
        {	
            const image = await this.setImageValue(gridImages[i]);
            this.gridImages.push(image);
        }

        console.log('Finished loading grid');
    }

    public async setupExplosions(): Promise<void> {
        await this.calculateSpriteAnimations("Explosion1", default_explosion_sheet, this.explosions, 50, 50);
		await this.calculateSpriteAnimations("Explosion2", nuclear_explosion_sheet, this.explosions, 50, 50);

        console.log('Finished loading explosions');
    }

    public async setupBullets(): Promise<void> {
        await this.calculateRotateImagesAsync("BasicBullet", imgBullet_Basic, this.bullets, 32 , 32);
		await this.calculateRotateImagesAsync("Rocket", imgRocket_Basic, this.bullets, 50 , 50);
		await this.calculateRotateImagesAsync("NuclearGrenade", nuclear_bullet, this.bullets, 50 , 50);

        console.log('Finished loading bullets');
	};

    public async setupAssets(): Promise<void> {
        this.imgSurrounder = await this.calculateImage(imgBlackTransparentSrc);
        this.imgBlackTransparent = await this.calculateImage(imgSurrounderSrc);
    }

    public setImageValue(imageBase64: string): Promise<HTMLImageElement>
	{
        var image = new Image();

        const imageCanvas = createCanvas();
        const imageCtx = imageCanvas.getContext('2d') as CanvasRenderingContext2D;

        if (!imageCtx || !imageCanvas) {
            throw new Error("Canvas context or canvas is not initialized.");
        }
        return new Promise((resolve, reject) => {
            image.onload = function(){
                imageCtx.clearRect(0, 0, 50, 50);
                imageCtx.save();
                imageCtx.drawImage(image, 0, 0, 50, 50);
                imageCtx.restore();

                var saveImage = new Image();

                saveImage.onload = function(){
                    resolve(saveImage);				 
                };
                saveImage.onerror = (error) => {
                    reject(new Error("Error loading save image: " + error));
                };

                saveImage.src = imageCanvas.toDataURL();
            };

            image.onerror = (error) => {
                reject(new Error("Error loading source image: " + error));
            };

            image.src = imageBase64;
        });

	}

    public async calculateImage(imagesrc: string): Promise<HTMLImageElement> {
        const image = new Image();

        const imageCanvas = createCanvas();
        const imageCtx = imageCanvas.getContext('2d') as CanvasRenderingContext2D;

        if (!imageCtx || !imageCanvas) {
            throw new Error("Canvas context or canvas is not initialized.");
        }
    
        return new Promise((resolve, reject) => {
            image.onload = () => {
                imageCtx.clearRect(0, 0, 50, 50);
                imageCtx.save();
                imageCtx.drawImage(image, 0, 0, 50, 50);
                imageCtx.restore();
    
                const saveImage = new Image();
                saveImage.onload = () => {
                    resolve(saveImage);
                };
                saveImage.onerror = (error) => {
                    reject(new Error("Error loading save image: " + error));
                };
                saveImage.src = imageCanvas.toDataURL();
            };
    
            image.onerror = (error) => {
                reject(new Error("Error loading source image: " + error));
            };
    
            image.src = imagesrc;
        });
    }

    public async calculateSpriteAnimations(name: string, imagesrc: string, destination: RotateImage[], drawheight = 50, drawwidth = 50): Promise<void>
	{
		var images: HTMLImageElement[] = [];
		var source_sheet = new Image();

        const imageCanvas = createCanvas();
        const imageCtx = imageCanvas.getContext('2d') as CanvasRenderingContext2D;

        if (!imageCtx || !imageCanvas) {
            throw new Error("Canvas context or canvas is not initialized.");
        }

        await new Promise<void>((resolve, reject) => {
            source_sheet.onload = function(){            
                //Sheet is vertical.
                //var clipped = 0;
                clipImage(0);
                function clipImage(clipped: number)
                {
                    if(clipped != (source_sheet.width / drawwidth))
                    {
                        clipped++;

                        imageCtx.clearRect(0, 0, 50, 50);
                        imageCtx.save();
                        imageCtx.drawImage(source_sheet, drawwidth * clipped ,0, 50,50,0, 0, drawwidth, drawheight);
                        imageCtx.restore();

                        var saveImage = new Image();
                        saveImage.onload = function(){

                            images.push(saveImage);
                            clipImage(clipped);
                        };
                        saveImage.onerror = (err) => reject(err);                        
                        saveImage.src = imageCanvas.toDataURL();
                    }
                    else {
                        resolve();
                    }
                }					
            };

            source_sheet.src = imagesrc;

        });


       	var item = {
	                    name : name,
	                    images : images, 
	               };
	    destination.push(item);  
	}

    public async calculateRotateImagesAsync(
        name: string,
        imagesrc: string,
        destination: RotateImage[],
        drawheight: number = 50,
        drawwidth: number = 50
    ): Promise<void> {
        const images: HTMLImageElement[] = [];
        const image = new Image();

        const imageCanvas = createCanvas();
        const imageCtx = imageCanvas.getContext('2d') as CanvasRenderingContext2D;
    
        if (!imageCtx || !imageCanvas) {
            throw new Error("Canvas context or canvas is not initialized.");
        }
    
        await new Promise<void>((resolve, reject) => {
            image.onload = () => {
                const rotate = async (i: number): Promise<void> => {
                    imageCtx.clearRect(0, 0, drawheight, drawwidth);
                    imageCtx.save();
                    imageCtx.translate(drawheight / 2, drawwidth / 2);
                    imageCtx.rotate(i * TO_RADIANS);
                    imageCtx.drawImage(image, -(drawheight / 2), -(drawwidth / 2), drawheight, drawheight);
                    imageCtx.restore();
    
                    const saveImage = new Image();
                    saveImage.onload = () => {
                        images.push(saveImage);
    
                        if (images.length < 360) {
                            rotate(i + 1);
                        } else {
                            resolve();
                        }
                    };
    
                    saveImage.onerror = (err) => reject(err);
                    saveImage.src = imageCanvas.toDataURL();
                };
    
                rotate(0);
            };
    
            image.onerror = (err) => reject(err);
            image.src = imagesrc;
        });
    
        const towerImage = {
            name: name,
            images: images,
        };
    
        destination.push(towerImage);
    }
    
    public addTower(src: string, index: number, moneys: number){
		const div = document.getElementById('turrets') as HTMLDivElement;
		var towerTemplate = '<div style="width:50px;float:left;margin:5px;" class="selectImage" cost="'+ moneys + '" index="'+ index + '"><img src="' + src + '" style="backgrond:red;" width="50" height="50"><div width="50"><div style="display: table;margin: 0 auto;">'+moneys+'<img src ="./images/money.png" style="vertical-align: middle;margin-left:3px;" width="15" height="15"></div></div></div>';
		div.innerHTML +=  towerTemplate;
	};
}

const imgTurret_Basic = "data:image/png;base64,";
const imgRocketLauncher_Basic = "data:image/png;base64,";
const nuclear_turret = "data:image/png;base64,";
const multilauncher = "data:image/png;base64,";
const imgEnemy_Basic = "data:image/png;base64,";
const imgBullet_Basic = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAAeklEQVRYR2NkGGDAOMD2M4w6YDQERkOAohBYLPnlPygbxz7nIdscsjXCLIeVI+Q6YtQBQzMEQPG/97YqSjXirHqbrMRIcghgsxzmEnIcMeqAoRcCoPge0EQIS3CjJeFoCMAS44BVx9RqzpNcDlDLYpg5ow4YDYHREAAAvzpSISuYp5oAAAAASUVORK5CYIIA";
const imgRocket_Basic = "data:image/png;base64,";
const nuclear_bullet = "data:image/png;base64,";
const imgSurrounderSrc = "data:image/png;base64,";
const imgBlackTransparentSrc = "data:image/png;base64,";


const gridImages: string[] = [];
gridImages.push("data:image/png;base64,");
gridImages.push("data:image/png;base64,");
gridImages.push("data:image/png;base64,");
gridImages.push("data:image/png;base64,");
gridImages.push("data:image/png;base64,");
gridImages.push("data:image/png;base64,");
gridImages.push("data:image/png;base64,");
gridImages.push("data:image/png;base64,");
gridImages.push("data:image/png;base64,");
gridImages.push("data:image/png;base64,");
gridImages.push("data:image/png;base64,");
gridImages.push("data:image/png;base64,");
gridImages.push("data:image/png;base64,");
gridImages.push("data:image/png;base64,");
gridImages.push("data:image/png;base64,");


const default_explosion_sheet = "data:image/png;base64,"
const nuclear_explosion_sheet = "data:image/png;base64,";

const createOffScreenCanvas = (): OffscreenCanvas => {
    const offscreen = new OffscreenCanvas(50, 50);
    return offscreen
};

const createCanvas = (): HTMLCanvasElement => {
    const canvas = document.createElement('canvas');
    canvas.id = 'canvas_' + Math.random().toString(36).substring(2, 15);;
    canvas.style.display = 'none'; // Hide canvas as per the requirement
    canvas.width = 50;
    canvas.height = 50;
    document.body.appendChild(canvas);
    return canvas;
};