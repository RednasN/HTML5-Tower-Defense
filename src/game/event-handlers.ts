...
export class EventHandlers {
    ...
    constructor(canvasState: TwdCanvasState, twdGrid: TowerDefenseGrid, gameLoop: GameLoop, twdMenu: TwdMenu) {
    ...
    private getClickedCell(): Cell | null {
        ...
        for (let x = 0; x < this.twdGrid.grid.length; x++) {
                for (let y = 0; y < this.twdGrid.grid[x].length; y++) {
                    var startx = this.twdGrid.grid[x][y].drawx + this.canvasState.mainCanvasXOffset;
                    var starty = this.twdGrid.grid[x][y].drawy + this.canvasState.mainCanvasYOffset;

                    if (this.canvasState.mainCanvasXCurrent > startx  && this.canvasState.mainCanvasXCurrent < (startx + this.twdGrid.grid[x][y].width) &&
                    this.canvasState.mainCanvasYCurrent > starty && this.canvasState.mainCanvasYCurrent < (starty + this.twdGrid.grid[x][y].height)) {
    ...
    public calculateUpgradeCosts(): void
	{
		...
		speedCostElement.innerHTML = Math.round((parseInt(speedCostElement.innerHTML) + 1) * (this.twdMenu.towercost / 5)).toString();
        rangeCostElement.innerHTML = Math.round((parseInt(rangeCostElement.innerHTML) + 1) * (this.twdMenu.towercost / 5)).toString();
        powerCostElement.innerHTML = Math.round((parseInt(powerCostElement.innerHTML) + 1) * (this.twdMenu.towercost / 5)).toString();
		...
		if(parseInt(speedValueElement.innerHTML) != this.twdMenu.speedLevel)
		{
			...
			this.twdMenu.towerSpeedCost += (this.twdMenu.towercost / 5) * this.twdMenu.speedLevel;			
		}

		if(parseInt(powerCostElement.innerHTML) != this.twdMenu.powerLevel)
		{	
			this.twdMenu.powerLevel = parseInt(powerCostElement.innerHTML);
			this.twdMenu.towerPowerCost += (this.twdMenu.towercost / 5) * this.twdMenu.powerLevel;			
		}
		...
		if(parseInt(rangeValueElement.innerHTML) != this.twdMenu.rangeLevel)
		{
			...
			this.twdMenu.towerRangeCost += (this.twdMenu.towercost / 5) * this.twdMenu.rangeLevel;			
		}
		...
		totalCostElement.innerHTML = (this.twdMenu.towercost + this.twdMenu.towerSpeedCost + this.twdMenu.towerPowerCost + this.twdMenu.towerRangeCost).toString();    	
	}
    ...
    public minusSpeed(): void {
		...
		{
			speedValueElement.innerHTML = (parseInt(speedValueElement.innerHTML) - 1).toString();
	...
	public minusRange(): void {
        ...
        {
            rangeValueElement.innerHTML = (parseInt(rangeValueElement.innerHTML) - 1).toString();
            this.calculateUpgradeCosts();
        }
	}
    ...
    public minusPower(): void {
        ...
        {
            powerValueElement.innerHTML = (parseInt(powerValueElement.innerHTML) - 1).toString();
            this.calculateUpgradeCosts();
        }
    }
    ...
    public changeSpeed(): void {    
        ...
        const gamespeedImage = document.getElementById('gamespeed') as HTMLImageElement;
        if(gamespeedImage) gamespeedImage.src = "./images/speed-level-"+ this.gameLoop.speed+".svg";
    }

    public changePause(): void {
        this.gameLoop.changePause();

        const gamestateImage = document.getElementById('gamestate') as HTMLImageElement;
        if(gamestateImage) gamestateImage.src = "./images/" + (this.gameLoop.paused ? "pause.svg" : "play.svg");
    }
    ...
    public selectImage(e: Event): void {
        ...
        if (towerIndex && cost) { // Ensure attributes are not null
    ...
    public setupMainHandlers(): void {
        ...
        const minRangeElements = document.getElementsByClassName("plusRange");
        ...
        const minSpeedElements = document.getElementsByClassName("plusSpeed");
        ...
        const closeBuildElements = document.getElementsByClassName("closeBuildModal");
		...
		this.canvasState.mainCanvas.addEventListener('touchstart', (e) => {	
			...
			this.canvasState.mainCanvasYCurrent = e.targetTouches[0].pageY / this.canvasState.scaleToFit;
            this.canvasState.mainCanvasXCurrent = e.targetTouches[0].pageX / this.canvasState.scaleToFit;            
			...
			if(this.lastClicked != null)
			{
				...
				if(diff < 300)
				{
					...
					if(this.clickedCell !== null && nowClickedCell !== null && nowClickedCell.x  == this.clickedCell.x && this.clickedCell.y == nowClickedCell.y)
					{
						this.twdMenu.buildx = nowClickedCell.x;
						this.twdMenu.buildy = nowClickedCell.y;
		...
		this.canvasState.mainCanvas.addEventListener('touchmove', (e) => {
            ...
            if(this.twdMenu.draggedTurret == null)
            {
				var diffY = e.targetTouches[0].pageY / this.canvasState.scaleToFit - this.canvasState.mainCanvasYCurrent;
	            this.canvasState.mainCanvasYCurrent += diffY;
	            this.canvasState.mainCanvasYOffset += diffY;

	            var diffX = e.targetTouches[0].pageX / this.canvasState.scaleToFit - this.canvasState.mainCanvasXCurrent;
	            this.canvasState.mainCanvasXCurrent += diffX;
	            this.canvasState.mainCanvasXOffset += diffX;

	            this.canvasState.mainCanvasYOffset = Math.floor(this.canvasState.mainCanvasYOffset);
	            this.canvasState.mainCanvasXOffset = Math.floor(this.canvasState.mainCanvasXOffset);
        	}
